...
// rlottie-wasm.js is a classic IIFE script; use dynamic import via fetch
// to be compatible with ES module workers (format: 'es' in vite config)
const rlottieWasmUrl = new URL('./rlottie-wasm.js', import.meta.url).href;
await import(/* @vite-ignore */ rlottieWasmUrl).catch(() => {
  // Fallback: some environments require importScripts (classic workers)
  // eslint-disable-next-line no-restricted-globals
  if (typeof importScripts === 'function') importScripts(rlottieWasmUrl);
});
